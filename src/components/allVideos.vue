      ...
      <el-table :data="tableData" style="width: 100%">
        ...
        <el-table-column label="操作" width="500">
          <template slot-scope="scope">
            ...
            >{{scope.row.publish === true?"取消发布":"视频发布"}}</el-button>
            <el-button
              size="mini"
              type="primary"
            ...
            <el-button size="mini" type="success" @click="handlePlayCount(scope.row.id)">播放量+1</el-button>
...
export default {
  ...
  mounted() {
    this.axios
      .get("https://www.lfeng.site/taskgo/app/video?curPage=1&pageSize=5")
      .then(res => {
        console.log(res);
  ...
  methods: {
    handleSizeChange(size) {
      ...
      this.axios
        .get(
          "https://www.lfeng.site/taskgo/app/video" +
            that.publishValue +
            "?curPage=1&pageSize=" +
            size
        )
    ...
    handlePublish(id) {
      ...
      this.$confirm("此操作将发布此音频, 是否继续?", "提示", {
        ...
        .then(() => {
          this.axios
            .put(
              "https://www.lfeng.site/taskgo/app/video/publish" +
                that.publishValue +
                "?videoId=" +
                id +
                "&publish=true"
    ...
    handleNoPublish(id) {
      ...
      this.$confirm("此操作将取消发布此音频, 是否继续?", "提示", {
        ...
        .then(() => {
          this.axios
            .put(
              "https://www.lfeng.site/taskgo/app/video/publish?videoId=" +
                id +
                "&publish=false"
            )
            .then(() => {
    ...
    handleTask(id) {
      ...
      this.$confirm("此操作将把此音频转为任务, 是否继续?", "提示", {
        ...
        .then(() => {
          this.axios
            .put(
              "https://www.lfeng.site/taskgo/app/video/covert?videoId=" +
                id +
                "&publish=false"
            )
            .then(() => {
    ...
    handlePlayCount(id) {
      let that = this;
      this.axios
        .put("https://www.lfeng.site/taskgo/app/video/playCount?videoId=" + id)
        .then(() => {
          this.axios
            .get(
              "https://www.lfeng.site/taskgo/app/video" +
    ...
    searchPublish() {
      ...
      this.axios
        .get(
          "https://www.lfeng.site/taskgo/app/videos" +
            that.publishValue +
